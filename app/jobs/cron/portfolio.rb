  ...
  class Portfolio
    ...
    class <<self
      ...
      def process_order(portfolio_currency, liability_id, trade, order)
        ...
        total_credit_value = (total_credit + total_credit_fees) * price_at(portfolio_currency, income_currency_id, trade.created_at)
      ...
      def process_deposit(portfolio_currency, liability_id, deposit)
        ...
        total_credit_value = (total_credit + total_credit_fees) * price_at(portfolio_currency, deposit.currency_id, deposit.created_at)
      ...
      def process_adjustment(portfolio_currency, liability_id, adjustment)
        Rails.logger.info { "Process adjustment: #{adjustment.id}" }

        total_credit = adjustment.amount
        total_credit_value = total_credit * price_at(portfolio_currency, adjustment.currency_id, adjustment.created_at)
        portfolios_values(adjustment.creator_id, portfolio_currency, adjustment.currency_id, total_credit, 0, total_credit_value, liability_id, 0, 0, 0)
      end
          ...
          .each do |liability|
            ...
            case liability['reference_type']
              when 'Adjustment'
                ...
                values << process_adjustment(portfolio_currency, liability['id'], adjustment)
              when 'Deposit'
