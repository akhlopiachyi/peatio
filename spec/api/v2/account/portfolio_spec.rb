...
describe API::V2::Account::Portfolio, type: :request do
  ...
  describe 'GET /api/v2/account/portfolio' do
    ...
    it 'returns all user portfolios for all portfolio currencies' do
      ...
      expect(response_body[0]['average_buy_price'].to_f.round(9)).to eq( (portfolio1.total_credit_value / (portfolio1.total_credit + portfolio1.total_credit_fees)).to_f)
      expect(response_body[0]['average_sell_price'].to_f.round(9)).to eq(portfolio1.total_debit_value / (portfolio1.total_debit + portfolio1.total_debit_fees))
      ...
      expect(response_body[1]['average_buy_price'].to_f.round(9)).to eq( (portfolio2.total_credit_value / (portfolio2.total_credit + portfolio2.total_credit_fees)).to_f)
      expect(response_body[1]['average_sell_price'].to_f.round(9)).to eq(portfolio2.total_debit_value / (portfolio2.total_debit + portfolio2.total_debit_fees))
    end
    ...
    context 'avarage sell price equal to 0' do
      ...
      it 'return user portfolio with zero avarage sell price' do
        ...
        expect(response_body[0]['average_buy_price'].to_f.round(9)).to eq( (portfolio.total_credit_value / (portfolio.total_credit + portfolio.total_credit_fees)).to_f)
