...
describe Jobs::Cron::Portfolio do
  ...
  context 'process_deposit' do
    ...
    context 'coin deposit' do
      ...
      it do
        ...
        expect(result[6].to_f).to eq (coin_deposit.amount + coin_deposit.fee) * trade1.price
    ...
    context 'fiat deposit' do
      ...
      it do
        ...
        expect(result[6].to_f).to eq (fiat_deposit.amount + fiat_deposit.fee) * trade2.price
  ...
  context 'process_adjustment' do
    let!(:adjustment) { create(:adjustment, currency_id: 'btc')}
    let!(:liability) { create(:liability, member: member, credit: 0.4, reference_type: 'Adjustment', reference_id: adjustment.id) }

    let(:trade) { create(:trade, :btceth, price: '5.0'.to_d, amount: '1.9'.to_d, total: '5.5'.to_d)}

    before do
      Jobs::Cron::Portfolio.stubs(:price_at).returns(trade.price)
    end

    it do
      result = Jobs::Cron::Portfolio.process_adjustment(trade.market.quote_unit, liability.id, adjustment)
      result = result.split(/[(,)]/)
      expect(result[1].to_i).to eq adjustment.creator_id
      expect(result[2]).to eq "'#{trade.market.quote_unit}'"
      expect(result[3]).to eq "'#{adjustment.currency_id}'"
      expect(result[4].to_f).to eq adjustment.amount
      expect(result[5].to_f).to eq 0
      expect(result[6].to_f).to eq adjustment.amount.to_f * trade.price.to_f
      expect(result[7].to_i).to eq liability.id
      expect(result[8].to_i).to eq 0
      expect(result[9].to_i).to eq 0
      expect(result[10].to_i).to eq 0
    end
  ...
  context 'process_order' do
    ...
    context 'buy order' do
      it do
        ...
        expect(res1[6].to_f).to eq trade.amount.to_f * trade_eth.price
    ...
    context 'sell order' do
      it do
        ...
        expect(res1[5].to_f).to eq total_fees
        expect(res1[6].to_f).to eq trade.total * trade_eth.price
        expect(res1[7].to_i).to eq liability.id
        expect(res1[8].to_f).to eq 0
        expect(res1[9].to_f).to eq 0
        expect(res1[10].to_f).to eq 0
    ...
    context 'reference type deposit' do
      ...
      context 'creates one portfolio' do
        ...
        it do
          ...
          expect(Portfolio.last.total_credit_value).to eq (coin_deposit.amount + coin_deposit.fee) * trade_btceth.price
          expect(Portfolio.last.last_liability_id).to eq liability.id
        end
      end
      ...
      context 'creates several portfolios' do
        ...
        it do
          ...
          expect(Portfolio.second.total_credit_value).to eq (coin_deposit.amount + coin_deposit.fee) * trade_btceth.price
          ...
          expect(Portfolio.last.total_credit_value).to eq (fiat_deposit.amount + fiat_deposit.fee) * trade_btceth.price
      ...
      context 'calculation on existing portfolio' do
        ...
        it do
          ...
          expect(Portfolio.last.total_credit_value).to eq (coin_deposit.amount + coin_deposit.fee) * trade_btceth.price +  portfolio.total_credit_value
          expect(Portfolio.last.last_liability_id).to eq liability.id
        end
      end
    end

    context 'reference type adjustment' do
      context 'creates one portfolio' do
        let!(:adjustment) { create(:adjustment, currency_id: 'btc')}
        let!(:liability) { create(:liability, member: member, credit: 0.4, reference_type: 'Adjustment', reference_id: adjustment.id) }

        let(:trade_btceth) { create(:trade, :btceth, price: '5.0'.to_d, amount: '1.9'.to_d, total: '5.5'.to_d)}

        before do
          Jobs::Cron::Portfolio.stubs(:price_at).returns(trade_btceth.price)
        end

        it do
          expect{ Jobs::Cron::Portfolio.process_currency(trade_btceth.market.quote_unit) }.to change{ Portfolio.count }.by(1)
          expect(Portfolio.last.member_id).to eq adjustment.creator_id
          expect(Portfolio.last.currency_id).to eq adjustment.currency_id
          expect(Portfolio.last.portfolio_currency_id).to eq trade_btceth.market.quote_unit
          expect(Portfolio.last.total_credit).to eq adjustment.amount
          expect(Portfolio.last.total_debit).to eq 0
          expect(Portfolio.last.total_debit_value).to eq 0
          expect(Portfolio.last.total_debit_fees).to eq 0
          expect(Portfolio.last.total_credit_fees).to eq 0
          expect(Portfolio.last.total_credit_value).to eq adjustment.amount * trade_btceth.price
          expect(Portfolio.last.last_liability_id).to eq liability.id
        end
      end
      ...
      context 'calculation on existing portfolio' do
        let!(:adjustment) { create(:adjustment, currency_id: 'btc')}
        let!(:trade_btceth) { create(:trade, :btceth, price: '1.0'.to_d, amount: '0.3'.to_d, total: '5.5'.to_d)}
        let!(:portfolio) { create(:portfolio, currency_id: adjustment.currency_id,  portfolio_currency_id: trade_btceth.market.quote_unit, total_credit: 0.1, 
                                  total_credit_fees: 0.01, total_credit_value: 0.3, member_id: adjustment.creator_id, last_liability_id: 1)}
        let!(:liability) { create(:liability, id: 2, member_id: adjustment.creator_id, credit: 190.0, reference_type: 'Adjustment', reference_id: adjustment.id) }
        ...
        it do
          ...
          expect(Portfolio.last.portfolio_currency_id).to eq trade_btceth.market.quote_unit
          expect(Portfolio.last.currency_id).to eq adjustment.currency_id
          expect(Portfolio.last.total_credit).to eq (adjustment.amount + portfolio.total_credit)
          expect(Portfolio.last.total_debit_fees).to eq 0
          expect(Portfolio.last.total_credit_fees).to eq (0 + portfolio.total_credit_fees)
          expect(Portfolio.last.total_debit).to eq portfolio.total_debit
          expect(Portfolio.last.total_debit_value).to eq portfolio.total_debit_value
          expect(Portfolio.last.total_credit_value).to eq adjustment.amount * trade_btceth.price +  portfolio.total_credit_value
          expect(Portfolio.last.last_liability_id).to eq liability.id
        end
      end
    end
    ...
    context 'reference type trade' do
      context 'calculation on existing portfolio' do
        ...
        it do
          ...
          expect(Portfolio.all[1].total_credit_value).to eq trade.total * trade_btceth.price + portfolio2.total_credit_value
          ...
          expect(Portfolio.all[3].total_credit_value).to eq trade.amount * trade_btceth.price + portfolio4.total_credit_value
      ...
      context 'creates portfolios while executing 1 trade' do
        ...
        it do
          ...
          expect(Portfolio.all[1].total_credit_value).to eq trade.total * trade_btceth.price
          expect(Portfolio.all[1].last_liability_id).to eq liability.id
          ...
          expect(Portfolio.all[3].total_credit_value).to eq trade.amount * trade_btceth.price
          expect(Portfolio.all[3].last_liability_id).to eq liability.id
